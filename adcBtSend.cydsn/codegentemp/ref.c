...
#include "ref.h"
...
#if defined(ref__PC)
    #define ref_SetP4PinDriveMode(shift, mode)  \
    do { \
        ref_PC =   (ref_PC & \
                                (uint32)(~(uint32)(ref_DRIVE_MODE_IND_MASK << \
                                (ref_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (ref_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define ref_SetP4PinDriveMode(shift, mode)  \
        do { \
            ref_USBIO_CTRL_REG = (ref_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(ref_DRIVE_MODE_IND_MASK << \
                                    (ref_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (ref_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		ref_SetP4PinDriveMode(ref__0__SHIFT, mode);
    }
#endif
...
void ref_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  ref_INTCFG & (uint32)(~(uint32)position);
    ref_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
